...
export default function Home() {
  ...
  return <main>
    ...
    <section className="category-grid" aria-label="Kategori katalog">{activeCategories.map((entry) => <button className="category-card" key={entry.id} onClick={() => { setCategory(entry); setCategorySearch(""); setCategorySize(""); setItem(null); }}><img src={entry.imageUrl} alt={entry.name} loading="lazy" /><span className="category-overlay"><small>Koleksi</small><strong>{entry.name}</strong><i>Jelajahi →</i></span></button>)}</section>
    ...
    {category && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={`Koleksi ${category.name}`} onMouseDown={(e) => e.target === e.currentTarget && setCategory(null)}><section className="catalog-modal"><button className="close" onClick={() => setCategory(null)} aria-label="Tutup">×</button><div className="modal-heading"><button onClick={() => setCategory(null)}>← Semua kategori</button><div className="category-heading-row"><div><p className="eyebrow">{category.name}</p><h2>{category.description}</h2></div><div className="category-tools"><label className="category-search"><span>⌕</span><input value={categorySearch} onChange={(e) => setCategorySearch(e.target.value)} placeholder="Cari dalam kategori…" aria-label="Cari dalam kategori" /></label><label className="category-size-filter"><select value={categorySize} onChange={(e) => setCategorySize(e.target.value)} aria-label="Filter berdasarkan ukuran"><option value="">Filter by Size</option>{availableCategorySizes.map((size) => <option value={size} key={size}>{size}</option>)}</select></label></div></div></div>{categorySubgroups.length ? <div className="subcategory-sections">{categorySubgroups.map(({ sub, items }) => <section className="subcategory-section" key={sub.id}><div className="subcategory-title"><span>{String(sub.sortOrder).padStart(2, "0")}</span><h3>{sub.name}</h3><i>{items.length} koleksi</i></div><div className="item-grid">{items.map((entry) => <ItemCard key={entry.id} entry={entry} wished={wishlist.includes(entry.id)} openItem={openItem} toggleWishlist={toggleWishlist} />)}</div></section>)}</div> : <p className="empty">Tidak ada item yang cocok.</p>}</section></div>}
...
function ItemCard({ entry, wished, openItem, toggleWishlist }: { entry: Item; wished: boolean; openItem: (item: Item) => void; toggleWishlist: (id: string, event?: MouseEvent) => void }) {
  const cardSpecs = [entry.color, entry.motif, entry.size].filter((value) => Boolean(String(value || "").trim()));
  return <article className="item-card"><button className="item-open" onClick={() => openItem(entry)}><img src={entry.imageUrl} alt={entry.name} /><span className="item-card-copy"><strong>{entry.name}</strong>{cardSpecs.length > 0 && <span className="item-card-meta">{cardSpecs.join(" ∙ ")}</span>}</span></button><button className={`wish-button ${wished ? "active" : ""}`} onClick={(e) => toggleWishlist(entry.id, e)} aria-label={wished ? `Hapus ${entry.name} dari wishlist` : `Tambah ${entry.name} ke wishlist`}>{wished ? "♥" : "♡"}</button></article>;
}
