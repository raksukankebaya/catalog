  ...
  return <main>
    ...
    <section className="gallery-section" id="galeri">
      ...
      <div className={`gallery-results ${galleryCategoryId ? "visible" : ""}`} ref={galleryResultsRef}>
        {!galleryCategoryId ? <p className="gallery-prompt"> </p> : currentGalleryEntry ? <div className="gallery-focus-wrap">
          ...
          <div className={`gallery-focus-carousel ${galleryMotion ? `push-${galleryMotion}` : ""}`} onTouchStart={(event) => { galleryTouchStartX.current = event.touches[0].clientX; }} onTouchEnd={(event) => finishGallerySwipe(event.changedTouches[0].clientX)}>
            ...
            <button key={`current-${currentGalleryEntry.id}`} className="gallery-focus-card current" onClick={() => setGalleryItem(currentGalleryEntry)} aria-label={`Buka ${currentGalleryEntry.title}`}><img src={galleryThumb(currentGalleryEntry)} alt={currentGalleryEntry.title} />{currentGalleryEntry.mediaType === "video" && <span className="play-mark">▶</span>}<span className="gallery-focus-copy"><small>{currentGalleryEntry.mediaType === "video" ? "Video" : "Foto"}</small><strong>{currentGalleryEntry.title}</strong><i> </i></span></button>
...
function Area({ label, value, onChange, rows = 4 }: { label: string; value: unknown; onChange: (v: string) => void; rows?: number }) { return <label>{label}<textarea value={String(value ?? "")} onChange={(e) => onChange(e.target.value)} rows={rows} /></label>; }
