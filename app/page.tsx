...
export default function Home() {
  ...
  return <main>
    ...
    <footer id="kontak"><div className="footer-brand"><img src="logo.png" alt="" /><div><h2>{data.contact.brand || "Raksukan Kebaya"}</h2><p>{data.contact.tagline}</p></div></div><div><h3>Hubungi kami</h3><a href={`https://wa.me/${whatsapp}`} target="_blank">WhatsApp · {data.contact.phone}</a><a href={`mailto:${data.contact.email}`}>{data.contact.email}</a><p>{data.contact.address}</p></div><div><h3>Media sosial</h3><a href={`https://instagram.com/${data.contact.instagram?.replace(/^@/, "")}`} target="_blank">Instagram · @{data.contact.instagram?.replace(/^@/, "")}</a><p>{data.contact.description}</p></div><small className="copyright">© {new Date().getFullYear()} Raksukan Kebaya. Seluruh hak dilindungi.</small></footer>
